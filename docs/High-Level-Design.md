...
Please refer to dependent 3rd party libraries, too. (GoogleProtobuf is necessary for ```Model``` representation, which will be replaced with other functions)  
...
When ```InferenceEngine``` is created, compiled model is loaded to one ```Model```, which is partitioned and converted to chain of ```Task```. ```Task``` represents NN operations information, and describes where(NPU or CPU) should it be done.  
...
Then, tensors are created based on the model parameters, and attached to each tasks. So, ```Worker``` instances will execute the task chain asynchronously as followings. As a result, selected ```Device``` by ```Worker``` will perform NN computations(if needed, memory write/read operations of tensors will be occured).  
![image](https://user-images.githubusercontent.com/79885630/199922947-5ca45502-6f27-4cec-97a0-710218bd8bc7.png)
## Compiler Interface
...
## Device Allocation
1. Device allocation by user  
InferenceOption configured by user will set what specific device to use (Please refer to ```InferenceOption``` usage in API description)  
2. Device allocation by DXRT  
If multiple devices are configured in ```InferenceOption```, ```Worker``` will allocate real-time available device for each requested task. (Please refer to above Inference Concept diagram)  
