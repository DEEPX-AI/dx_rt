...
static int dxrt_identify_device(struct dxdev* dev, dxrt_message_t *msg)
{
	...
	else
	{
		...
		if(dev->msg)
		{
			...
			if(ret<0) return -1;
			memcpy(&info, &dev->msg->data, sizeof(info));
			pr_debug("%d: %s: [%llx, %llx], %d\n", num, __func__,
				info.mem_addr, info.mem_size,
				info.num_dma_ch);
		...
		return ret;
        // int ch;
        // ret = dx_pcie_send_message(
        //     DX_PCIE_CMD_IDENTIFY_DEVICE,
        //     DX_MSG_RECV,
        //     sizeof(dxrt_device_info_t),
        //     num,
        //     (void *)&info
        // );
        // if(ret<0) return -1;
	...
	if(dev->type==0)
	{
		...
		if(msg->data!=NULL)
		{
			...
			if (req.input.data)
			{
				...
				{
					// pr_info( MODULE_NAME "%d: %s: ch %d, req %d, type %d, [%d cmds @ %x, weight @ %x], [%d cmds @ %x, weight @ %x], input @ %llx+%x(%x), output @ %llx+%x(%x), %x\n", 
					//     num, __func__, req.dma_ch,
					//     req.req_id, req.model_type, 
					//     req.model_cmds, req.cmd_offset, req.weight_offset,
					//     req.model_cmds2, req.cmd_offset2, req.weight_offset2,
	...
	if(dev->type==0)
	{
		...
		if(msg->data!=NULL)
		{
			...
			if(dx_pcie_is_response_queue_empty(num, ch))
			{
				...
				mask = dx_pcie_interrupt(num, 2*ch+1); // 0 -> 1, 1 -> 3
...
static int dxrt_terminate(struct dxdev* dev, dxrt_message_t* msg)
{
	...
	{		
		...
		if(msg->data!=NULL)
		{
			...
			mask = dx_pcie_interrupt_wakeup(num, 2*ch+1); // 0 -> 1, 1 -> 3
		}
		return mask;
	}
	else
...
static int dxrt_update_firmware(struct dxdev* dev, dxrt_message_t* msg)
{
	...
	{
		dxrt_device_message_t *dev_msg = dev->msg;	
		dxrt_meminfo_t meminfo = {msg->data, 0xd8110000, 0, (msg->size>0xef000) ? 0xef000 : msg->size };
