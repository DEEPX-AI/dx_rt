...
static u32 read_interrupts(struct xdma_dev *xdev)
{
	...
	dbg_io("ioread32(0x%p) returned 0x%08x (user_irq_service).\n",
...
// static irqreturn_t user_irq_service(int irq, struct xdma_user_irq *user_irq) -> origin
static irqreturn_t user_irq_service(int irq, struct xdma_user_irq *user_irq, int user_irq_num)
{
	unsigned long flags;
	...
	/* DXNN START */
	if (user_irq_num == 1 || user_irq_num == 2) {
		send_signal_to_app(irq >> 1);
	}
	/* DXNN END */
...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	if (user_irq) {
		...
		for (; user < max && user_irq; user++, mask <<= 1) {
			if (user_irq & mask) {
				pr_info(" ** irq = %d, user_irq = 0x%08x\n", irq, user_irq);
				user_irq &= ~mask;
				user_irq_service(irq, &xdev->user_irq[user], user_irq);
			}
		}
	}
...
static irqreturn_t xdma_user_irq(int irq, void *dev_id)
{
	...
	return user_irq_service(irq, user_irq, 0);
}
...
#ifdef DMA_PERF_MEASURE
		dev_err(&xdev->pdev->dev, "[PERF] DMA Transer Elapsed Time(%s) @ %lld ns\n",
			(write == 0) ? "WRITE" : "READ",
