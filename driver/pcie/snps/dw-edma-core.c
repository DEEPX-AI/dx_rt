...
static void dx_user_irq_init(struct dw_edma *dw)
{
	...
	// dxrt
	dw->response[0] = dw->npu_region[0].vaddr + MSGRAM_RESPONSE0_OFFSET;
	dw->response[1] = dw->npu_region[0].vaddr + MSGRAM_RESPONSE1_OFFSET;
	INIT_LIST_HEAD(&dw->responses[0].list);
	INIT_LIST_HEAD(&dw->responses[1].list);
}
static inline
...
static irqreturn_t user_irq_service(int irq, struct dx_dma_user_irq *user_irq)
{
	...
	// dxrt
	{
		int ch = user_irq->user_idx / 2 ;
		if(ch==0 || ch==1)
		{
			struct dw_edma *dw = user_irq->dw;
			dx_pcie_response_list_t *entry;
			entry = kmalloc(sizeof(dx_pcie_response_list_t), GFP_KERNEL);
			if(!entry)
			{
				printk(KERN_ALERT "Failed to allocate memory for response\n");
			}
			else
			{
				*((dx_pcie_response_t*)&entry->response) = *((dx_pcie_response_t*)dw->response[ch]);
				// pr_info("dxrt%d: irq: %d\n", dw->idx, entry->response.req_id);
				// pr_info( "ch%d, res %d\n", ch, entry->response.req_id);
				// pr_info( "ch%d, argmax %d\n", ch, entry->response.argmax);
				// pr_info( "ch%d, status %d\n", ch, entry->response.status);
				spin_lock_irqsave(&dw->responses_lock[ch], flags);
				list_add_tail(&entry->list, &dw->responses[ch].list);
				spin_unlock_irqrestore(&dw->responses_lock[ch], flags);
			}
		}
	}
...
static int user_irq_check(int irq, void *data)
{
	...
	for(i = 0; i < NPU_MASK_MAX; i++) {
		...
		dbg_irq("NPU_%d[%s] : %d -> %d \n", 
			(int)(i/USER_NUM_MAX),  user_irq_vec_table[i].name,
			npu_irq_cnt[i],        temp);
