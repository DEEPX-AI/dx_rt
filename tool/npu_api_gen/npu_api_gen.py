...
class Memory:
    def __init__(self, list):
        ...
        assert len(list)==12, f"Please Check Excel( size : {len(list)} )"
...
if __name__ == '__main__':
    ...
    generate_template_module(input_excel, 'template/NpuReg.mako', f'{output_dir}/NpuRegSystem.h', system, npu_variant)
    generate_template_module(input_excel, 'template/NpuReg.mako', f'{output_dir}/NpuRegDebug.h', debug, npu_variant)
    generate_template_module(input_excel, 'template/NpuReg.mako', f'{output_dir}/NpuRegDma.h', dma, npu_variant)
    generate_template_modules(input_excel, 'template/NpuRegAPI.cpp.mako', f'{output_dir}/NpuRegAPI.cpp', 
        [system, debug, dma], npus , npu_variant)
    generate_template_modules(input_excel, 'template/NpuRegAPI.h.mako', f'{output_dir}/NpuRegAPI.h', 
        [system, debug, dma], npus , npu_variant)
    generate_template_modules(input_excel, 'template/NpuMem.cpp.mako', f'{output_dir}/NpuMem.cpp', 
        mems, npus , npu_variant)
    for idx in range(len(postfixes)):
        postfix = postfixes[idx]
        generate_template_module(input_excel, 'template/NpuReg.mako', f'{output_dir}/NpuRegNpu{idx}.h', npus[idx], npu_variant)
        # generate_template_module(input_excel, 'template/NpuMem.cpp.mako', f'{output_dir}/NpuMem{idx}.cpp', mems[idx])
