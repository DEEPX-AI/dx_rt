...
{
    DXRT_ASSERT(ipcClientWrapper.Initialize() == 0, "Failed to connect to dxrt memory manager service");
}
...
void MultiprocessMemory::SignalScheduller(int deviceId, const dxrt_request_acc_t& req)
{
    //unique_lock<mutex> _lk(_lock);
    dxrt::IPCClientMessage clientMessage;
    LOG_DXRT_DBG << "MultiprocessMemory::SignalScheduller(int deviceId)\n";
...
void MultiprocessMemory::SignalEndJobs(int deviceId)
{
    //unique_lock<mutex> _lk(_lock);
    dxrt::IPCClientMessage clientMessage;
    LOG_DXRT_DBG << "MultiprocessMemory::SignalEndJobs(int deviceId)\n";
