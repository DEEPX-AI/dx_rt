    ...
    for(auto &pair:_pool)
    {
        ...
        if(node.status==0)
        {
            if(required <= node.size)
            {
                ...
                // LOG_VALUE_HEX(ret);
                if(required < node.size)
                    _pool[addr + required] = MemoryNode{addr + required, node.size - required, 0};
                _pool[addr] = MemoryNode{addr, required, 1};
                return addr;
            }
        }
    }
...
void Memory::MergeAdjacentNodes(std::map<uint64_t, MemoryNode>::iterator it) {
    ...
    if (it != _pool.begin()) {
        ...
        if (prev_it->second.status == 0) {
            ...
            _pool.erase(prev_it);
            _pool.erase(it);
            _pool[new_addr] = MemoryNode{new_addr, new_size, 0};
