...
bool MemoryService::Deallocate(uint64_t addr, pid_t pid)
{
    ...
    {
        LOG_DXRT_S << "not regestered pid " << pid << " (legacy)" << endl;
        return false;
    }
    auto it2 = it1->second.find(addr);
    if (it2 == it1->second.end())
    {
        LOG_DXRT_S << "not allocated addr " << hex << addr << dec  << " for pid " << pid << " (legacy)" << endl;
        return false;
    }
...
void MemoryService::DeallocateAll(pid_t pid)
{
    ...
    if (taskIt != _taskAllocInfo.end()) {
        ...
        LOG_DXRT_S << "All task-based memory deallocated for PID:" << pid << endl;
    }
    ...
    if (legacyIt != _legacyAllocInfo.end()) {
        ...
        LOG_DXRT_S << "All legacy memory deallocated for PID:" << pid << endl;
    }
}
...
bool MemoryService::DeallocateTask(pid_t pid, int taskId)
{
    ...
    {
        LOG_DXRT_S << "Task deallocation: PID " << pid << " not found" << endl;
        return false;
    }
    ...
    {
        LOG_DXRT_S << "Task deallocation: Task " << taskId << " not found for PID " << pid << endl;
        return false;
    }
    ...
    LOG_DXRT_S << "All memory deallocated for Task " << taskId << ", PID:" << pid << endl;
    return true;
}
...
void MemoryService::DeallocateAllTasks(pid_t pid)
{
    ...
    LOG_DXRT_S << "All tasks memory deallocated for PID:" << pid << endl;
}
