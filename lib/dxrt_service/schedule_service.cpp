...
void SchedulerService::StopTaskInference(pid_t pid, int deviceId, int taskId)
{
    ...
    LOG_DXRT_S << "Stopping inference for PID " << pid << ", Device " << deviceId << ", Task " << taskId << endl;
    ...
    if (procIt != _map.end()) {
        ...
        LOG_DXRT_S << "Stopped " << requestsToRemove.size() << " inference requests for Task "
...
void SchedulerService::StopAllInferenceForProcess(pid_t pid, int deviceId)
{
    ...
    LOG_DXRT_S << "Stopping all inference for PID " << pid << ", Device " << deviceId << endl;
    ...
    LOG_DXRT_S << "Removed " << removedCount << " pending inference requests for process " << pid << endl;
}
