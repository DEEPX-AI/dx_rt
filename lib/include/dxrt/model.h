...
namespace deepx_rmapinfo {
    ...
    inline DataType GetDataTypeNum (const string& str) {
        if (str == "TYPE_NONE") return DATA_TYPE_NONE;
        ...
        if (str == "FLOAT32")   return FLOAT32;
    };

    inline DataFormat GetDataFormatNum (const string& str) {
        if (str == "DATAFORMAT_NONE") return DATAFORMAT_NONE;
        if (str == "NCHWd")           return NCHWd;
        if (str == "NHWCd")           return NHWCd;
        if (str == "PPU_YOLO")        return PPU_YOLO;
        if (str == "PPU_FD")          return PPU_FD;
        if (str == "PPU_POSE")        return PPU_POSE;
    };

    inline MemoryType GetMemoryTypeNum (const string& str) {
        if (str == "MEMORYTYPE_NONE") return MEMORYTYPE_NONE;
        if (str == "DRAM")            return DRAM;
        if (str == "ARGMAX")          return ARGMAX;
        if (str == "PPU")             return PPU;
    }

    inline InputMode GetInputModeNum (const string& str) {
        if (str == "INPUTMODE_NONE")  return INPUTMODE_NONE;
        if (str == "FORMATTER")       return FORMATTER;
        if (str == "IM2COL")          return IM2COL;
    }
} /* namespace deepx_rmapinfo */

namespace deepx_modelinfo {
...
deepx_modelinfo::ModelInfoDatabase LoadModelParam(std::string file);
