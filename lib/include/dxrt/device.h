...
public:
    ...
    // NFH worker accessor methods
    std::shared_ptr<NfhInputWorker> getNfhInputWorker() const { return _nfhInputWorker; }
    std::shared_ptr<NfhOutputWorker> getNfhOutputWorker() const { return _nfhOutputWorker; }
    std::shared_ptr<DeviceOutputWorker> getOutputWorker() const { return _outputWorker; }
    std::shared_ptr<DeviceInputWorker> getInputWorker() const { return _inputWorker; }

    // NFH worker protected member access wrapper
    int ProcessInferenceRequestACC(RequestData* req, npu_bound_op boundOp = N_BOUND_NORMAL)
    {
...
protected:
    ...
    std::shared_ptr<NfhInputWorker> _nfhInputWorker = nullptr;
    std::shared_ptr<NfhOutputWorker> _nfhOutputWorker = nullptr;
